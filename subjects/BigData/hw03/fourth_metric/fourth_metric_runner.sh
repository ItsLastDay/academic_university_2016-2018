...
STAT_OUT_DIR=${HDFS_DATA_DIR}/fourth_metric/${DATE}
...
function count_fourth_metric {
    ...
    if ! ${HDFS_COMMAND} -ls ${STAT_OUT_DIR}/_SUCCESS >/dev/null 2>&1; then
        ...
        echo "fourth metric ${date}"
        spark2-submit --master yarn ${SCRIPT_DIR}/fourth_metric.py ${date}
    fi
}
