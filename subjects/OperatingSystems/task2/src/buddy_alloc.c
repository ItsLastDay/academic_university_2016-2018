...
static void* buddy_alloc_pages_unlocked(uint64_t num_pages)
{
...
// allocated.
static void buddy_dealloc_pages_unlocked(void* addr, uint64_t num_pages)
{
    int order = get_needed_order(num_pages);
...
void* buddy_alloc_pages(uint64_t num_pages)
{
    ...
    void* result = buddy_alloc_pages_unlocked(num_pages);
...
{
    spin_lock(&buddy_lock);
    buddy_dealloc_pages_unlocked(addr, num_pages);
    spin_unlock(&buddy_lock);
}
