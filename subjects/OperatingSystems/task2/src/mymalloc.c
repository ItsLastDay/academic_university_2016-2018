...
static void* mymalloc_unlocked(size_t size)
{
    if (size == 0)
        return NULL;
...
void* mymalloc(size_t size)
{
    ...
    void* result = mymalloc_unlocked(size);
    spin_unlock(&malloc_lock);
    return result;
}


static void myfree_unlocked(void* addr)
{
